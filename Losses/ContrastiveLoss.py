...
class LossNetwork(torch.nn.Module):
    ...
    def forward(self, dehaze, gt, neg1, neg2):
        loss = []
        ...
        for i in range(len(dehaze_features)):
            ...
            neg1_gt = torch.exp(-self.L1(neg1_features[i], gt_features[i]) * 1 / 2)
            neg2_gt = torch.exp(-self.L1(neg2_features[i], gt_features[i]) * 1 / 2)
            ...
            #dehaze_input2 = torch.exp(-self.L1(dehaze_features[i], input2_features[i]) * 1 / 2)
            # dehaze_other_method = torch.exp(-self.L1(dehaze_features[i], other_method_features[i]) * 1 / 2)
            if dehaze_gt < neg1_gt and dehaze_gt < neg2_gt:
                per_loss = -torch.log(dehaze_gt / (dehaze_gt + dehaze_neg1 + dehaze_neg2))
            elif dehaze_gt < neg1_gt and dehaze_gt > neg2_gt:
                per_loss = -torch.log(dehaze_gt / (dehaze_gt + neg2_gt + 2 * dehaze_neg2 + dehaze_neg1))
            elif dehaze_gt > neg1_gt and dehaze_gt < neg2_gt:
                per_loss = -torch.log(dehaze_gt / (dehaze_gt + neg1_gt + 2 * dehaze_neg1 + dehaze_neg2))
            else:
                per_loss = -torch.log(dehaze_gt / (dehaze_gt + neg1_gt + 2 * dehaze_neg1 + neg2_gt + 2 * dehaze_neg2))
            # per_loss = dehaze_gt / (dehaze_input1 + 1e-7)
